		...
		<div class="col-md-8">
			<div class="box box-success">
				<div class="box-header with-border">
					...
					<small> <i class="fa fa-pencil"></i>&nbsp;${cultureNewsVO.press}
						&nbsp;|&nbsp; ${cultureNewsVO.nickname}
					</small> &nbsp;-&nbsp; <span class="description"> <i
						class="fa fa-clock-o"></i> <fmt:formatDate
							pattern="yyyy-MM-dd HH:mm" value="${cultureNewsVO.regdate}" />
						&nbsp;|&nbsp;<b>UPDATED</b> <fmt:formatDate
							pattern="yyyy-MM-dd HH:mm" value="${cultureNewsVO.uptdate}" />
					</span>
					<div class="box-tools pull-right">
						<button class='btn btn-box-tool' id='backBtn'><a href="/culture/list/">
							<i class='fa fa-chevron-left'></i></a>
						</button>
						<c:if test="${login.uid == cultureNewsVO.reporter_id }">
					...
					<br>
					<h1>${cultureNewsVO.title}</h1>
					<span class="description">${cultureNewsVO.subtitle}</span>
				</div>
				...
				<div class="box-body no-padding">
					<div class="mailbox-read-message">${cultureNewsVO.content}</div>
				...
				<div class="box-footer">
					<div>
						...
						<span class="pull-right text-muted"><span id="replyCnt">${cultureNewsVO.replycnt}</span>&nbsp;<i class="fa fa-commenting"></i></span>
				...
				<div class="box-footer">
					<div class="info-box">
						...
						<span class="text-muted">
							찬성:&nbsp;&nbsp;<span id = "positiveRate">${cultureNewsVO.vote_rate}</span>%&nbsp;&nbsp;|&nbsp;&nbsp;<span>${cultureNewsVO.votecnt_positive}명</span>
						</span>
						<span class="text-muted pull-right">
							반대:&nbsp;&nbsp;<span id = "negativeRate"></span>%&nbsp;&nbsp;|&nbsp;&nbsp;
							<span class="text-muted pull-right">${cultureNewsVO.votecnt_negative}명</span>
						</span>
						</div>
						<div class="info-box bg-light-blue-gradient"
							style="width: ${cultureNewsVO.vote_rate}%; float: left;">
...
<script>
	...
	$("#pagination").on("click", "li a", function(event) {
		...
		getPage("/culture_replies/" + cno + "/" + replyPage);
	});
...
<script>
	...
	$(document).ready(function() {
		...
		$("#modifyBtn").on("click", function() {
			formObj.attr("action", "/culture/modify");
		...
		$("#removeBtn").on("click", function() {
			...
			if(replyCnt > 0){
				...
				if (confirm(msg) != 0) {
					...
					if(arr.length > 0) {
						$.post("/culture_delAll", {files:arr}, function(){
							
						});
					}
					
					formObj.attr("action", "/culture/remove");
					formObj.submit();
				} else {
					return;
				}
...
/* 		$("#backBtn").on("click", function() {
		...
		$.getJSON("/culture/getAttach/" + cno, function(list){
			$(list).each(function(){
				
				var fileInfo = getCultureFileInfo(this);
		...
		function calcRate(){
			var negativeRate = 100 - ${cultureNewsVO.vote_rate};
